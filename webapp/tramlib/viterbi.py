...
## Caution: the matrix is transposed, use DataFrame.transpose to get right matrix
def to_dataframe(matrix):
    todict = {} ### we need to have the headline in the matrix too
    for tup in matrix: todict[tup[0]] = tup[1]
    todf = DataFrame(todict, index = todict[0]) # set headline as index
    return todf.transpose
...
def get_results(seq, transition, emission):
    ...
    pro_df = DataFrame(0, index= trans_states, columns= [*seq])
    pro_df.loc['s','start'] = 1 
    path = [] # save max prohibility in each column
    ...
    # Calculating
    # ---------------
    for symbol in pro_df.columns[1:]:
        for state in pro_df.index:
            a = 1
